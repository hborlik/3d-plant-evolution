...
public:
    ...
    std::list<std::shared_ptr<IGOT>> objs;
    ...
    void addDog()
    {
        vec3 tempPos = vec3(((rand()/(float)RAND_MAX) * 2.0) - 1.0f, ((rand()/(float)RAND_MAX) * 1.0) + 0.5f, ((rand()/(float)RAND_MAX) * 2.0) - 1.0f);
        vec3 tempVel = vec3(((rand()/(float)RAND_MAX) * 1.0 -.5), ((rand()/(float)RAND_MAX) * 0.0), ((rand()/(float)RAND_MAX) * 1.0 -.5));	
        std::shared_ptr<IGOT> someDog = std::make_shared<IGOT>();
        someDog->setPosition(tempPos);
        someDog->velocity = tempVel;
        someDog->setScale(vec3(0.1, 0.1, 0.1));
        someDog->sphere_radius = .25f;
        objs.push_back(someDog);
    }
