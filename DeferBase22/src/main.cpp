...
public:
	...
	void initGL(const std::string& resourceDirectory)
	{
		...
		texProg->addUniform("normalBuf");
		texProg->addUniform("colorBuf");
		texProg->addAttribute("vertPos");
		texProg->addUniform("Lpos");
	...
	void render(float frametime) {
		...
		if (DEFER & !FirstTime)
		{
			...
			texProg->bind();
				...
				glBindTexture(GL_TEXTURE_2D, gNormal);
				glUniform1i(texProg->getUniform("normalBuf"), 1);

				glActiveTexture(GL_TEXTURE2);
				glBindTexture(GL_TEXTURE_2D, gColorSpec);
				glUniform1i(texProg->getUniform("colorBuf"), 2);

				glUniform3f(texProg->getUniform("Lpos"), g_light.x, g_light.y, g_light.z);
	...
	void DrawQuad(GLuint inTex) {
		...
		glUniform1i(texProg->getUniform("texBuf"), 0);
		glUniform3f(texProg->getUniform("Lpos"), 1, -1, 0);
		glEnableVertexAttribArray(0);
		glBindBuffer(GL_ARRAY_BUFFER, quad_vertexbuffer);
		glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0, (void *) 0);
		glDrawArrays(GL_TRIANGLES, 0, 6);
